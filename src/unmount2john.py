...
class BitLockerMounter:
    ...
    def open_file_manager(self):
        ...
        # On Parrot, try sudo -u for file manager first
        if self.distro == 'parrot' and not opened:
            ...
            if original_user:
                ...
                for fm in file_managers:
                    try:
                        ...
                        which_result = subprocess.run(['which', fm[0]], capture_output=True, text=True)
                        if which_result.returncode == 0:
                            cmd = ['sudo', '-u', original_user] + fm
                            subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                            self.show_success(f"Opened with {fm[0]} as {original_user}")
                            opened = True
                            time.sleep(1)
                            break
                    except:
        ...
        # Try normal opening (works on Kali and others)
        if not opened:
            for fm in file_managers:
                try:
                    # Check if file manager exists
                    ...
                    if which_result.returncode == 0:
                        ...
                        self.show_success(f"Opened with {fm[0]}")
                        opened = True
                        time.sleep(1)
                        break
                except:
        ...
        if not opened:
            self.show_error("Could not open file manager automatically")
            print()
            self.show_info("Access manually:")
