...
class PDFCracker:
    def __init__(self):
        ...
        self.pdf2john_path = self.find_pdf2john()  # Auto-detect on init
        self.current_dir = os.getcwd()
        self.temp_dir = None
        
    def find_pdf2john(self):
        """Find pdf2john in common locations across different Linux distributions"""
        common_paths = [
            ...
            "/usr/local/share/john/pdf2john.py",
            "/usr/local/bin/pdf2john",
            
            # Source compilation
            "/opt/john/run/pdf2john.py",
            "/opt/john/pdf2john.py",
        ...
        print(tag_asterisk() + "Searching for pdf2john...")
        for path in common_paths:
            if os.path.exists(path):
                print(tag_plus() + f"Found at: {path}")
                return path
        
        print(tag_exclamation() + "pdf2john not found in common locations")
        print(tag_exclamation() + "pdf2john is usually part of John the Ripper Jumbo version")
        print(tag_exclamation() + "Install with: sudo apt install john  (Debian/Ubuntu)")
        return None
    ...
    def extract_hash(self, document_path):
        """Extract hash from PDF document"""
        ...
        self.temp_dir = tempfile.mkdtemp(prefix="pdf_hash_")
        self.hash_file = os.path.join(self.temp_dir, "pdf.hash")
        ...
        try:
            ...
            # Run pdf2john
            if self.pdf2john_path.endswith('.py'):
                cmd = [python_cmd, self.pdf2john_path, document_path]
            else:
            ...
            if result.returncode == 0 and result.stdout:
                ...
                self.show_error("pdf2john failed")
    ...
    def list_documents(self):
        ...
        print(BLUE + "\n____________________ " + GREEN + "Available PDF Documents" + BLUE + " ____________________\n")
    ...
    def select_document(self):
        ...
        if not documents:
            self.show_error("No PDF documents found!")
            print()
    ...
    def run_wordlist_attack(self, custom_wordlist=None):
        ...
        cmd = [self.john_path, self.hash_file, "--format=pdf", "--wordlist=" + wordlist]
    ...
    def run_incremental_mode(self):
        ...
        cmd = [self.john_path, self.hash_file, "--incremental=" + mode, "--format=pdf"]
    ...
    def main_loop(self):
        ...
        if not self.check_tools():
            print(tag_exclamation() + "Required tools not found. Please install john and pdf2john.")
            return
